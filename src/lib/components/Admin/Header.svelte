...
<header class="z-10 bg-transparent ">
  <div
    class="flex items-center justify-between px-6 py-2 mx-auto text-blue-600 dark:text-white"
  >
    <!-- Mobile hamburger -->
    <button
    ...
    {#if withBreadcrumb}
      <Breadcrumbs {items} let:item class="py-2 px-0">
        {#if item.href}
          <a class="s-breadcrumb-item text-sm text-teal-900 dark:text-white" href={item.href} class:disabled={item.disabled}>
            {item.text}
          </a>
        {/if}
      </Breadcrumbs>
