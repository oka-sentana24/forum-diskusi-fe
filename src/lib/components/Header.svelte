<script lang="ts">
   ...
   import { Avatar, Breadcrumbs,Card, } from 'svelte-materialify';
   import {
    isDark,
  ...
  import { List, ListGroup, ListItem, Icon, MaterialApp } from 'svelte-materialify';
...
<header class="absolute w-full md:w-[85%] z-50 md:py-4 bg-white border-b border-gray-300 dark:bg-gray-800 dark:border-b dark:border-gray-600">
      ...
      <div class="w-[50%] flex flex-cols-2 items-center">
        ...
        <div class='px-5'>
          <Breadcrumbs {items} let:item class="py-2 px-0">
            {#if item.href}
              <a
                class="s-breadcrumb-item text-sm text-gray-500 font-bold dark:text-white"
                href={item.href}
          ...
          <Avatar size="36px">
            ...
            <div
              use:clickOutside={['nav-profile-photo']}
							...
							aria-label="submenu"
            >
              <div class="absolute right-5 top-[74px]" transition:fly="{{ y: -10, duration: 500 }}">
