...
<div class="py-4 text-purple-500 dark:text-gray-600">
  {#if withTitle}
    <a class="ml-6  font-bold text-xs text-gray-800 dark:text-gray-200" href="/"> Windmill </a>
  {/if}
  <ul class="mt-6 ml-0">
    {#each links as link, a}
      <li class="relative px-6 py-3 hover:bg-purple-100 dark:hover:bg-gray-700">
        ...
        {#if !link.sublinks}
          <a
            class="inline-flex items-center w-full text-xs font-semibold transition-colors duration-150 dark:hover:text-gray-200"
        ...
        {:else}
          <button
            ...
            class="inline-flex items-center justify-between w-full text-xs  transition-colors duration-150 hover:text-gray-800 dark:hover:text-gray-200"
            aria-haspopup="true"
          >
          ...
          {#if $pageMenus[link.name]}
            <ul
              class="p-2 mt-2 space-y-2 overflow-hidden text-xs  font-normal text-gray-500 rounded-md shadow-inner bg-gray-50 dark:text-gray-400 dark:bg-gray-900"
              aria-label="submenu"
            >
  ...
  <div class="px-6 my-6">
    <button
      class="flex items-center justify-between w-full px-4 py-2 text-xs font-medium leading-5 text-red-100 transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple"
    >
      Create account
